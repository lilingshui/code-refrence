...
int main(int argc,char *argv[])
{
	...
	g_pLeInstance->Init();
	g_pLeInstance->RegisterSetCallback("beepSwitch", &(CBeeper::set_properties_callback_cb));
	g_pLeInstance->RegisterGetCallback("beepSwitch", &(CBeeper::get_properties_callback_cb));
	g_pLeInstance->RegisterServiceCallback("beepService", &(CBeeper::call_service_callback_cb));
	...
	while (1)
	{
		...
		strcpy(dev_proper_data.key, "beepSwitch");
