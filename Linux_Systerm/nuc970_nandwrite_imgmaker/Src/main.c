 ...
 *      main.c
 * DESCRIPTION
 *      Main program of nuc970ImgMaker.
 *      nuc970ImgMaker a NAND image that with valid format for Nuvoton NUC970 platform.
 *      Please note that NAND image don't handle bad block issue on NAND flash.
 * HISTORY
 *      2015/08/28  Create Ver 1.0.
 * REMARK
 *      None.
 *-----------------------------------------------------------------------------------*/
...
void usage()
{
    fprintf(stderr, "[Usage] ./nuc970ImgMaker\n"
            "	-v ....: version\n"
    ...
    fprintf(stderr, "\t[parameters]\n"
            "\t\t-t ....: Image type. Data:0, Environment:1, uBoot:2\n"
            ...
            "\t\t-g ....: Keep yaffs tags.\n"
            "\t\t-p ....: T24 bootstrap 368 OOB.\n"
            "\t\t-e ....: Execution address.\n\n" );

    fprintf(stderr, "[Example] ./nuc970ImgMaker -o ./nand_golden -p 2048 -o 64 -s 64 -b 1024\n" \
...
int LoadImgOption ( GOLDEN_INFO_T* psGoldenInfo, int i32No, char* strCmdlineOption, char *strImgName )
{
    ...
    const char* short_options = "t:b:o:S:O:je:g:p";
    const struct option long_options[] = {
        ...
        { "keepyaffstags",   required_argument,   NULL,    'g'     },
        { "t24bootstrap",     	no_argument,  		 NULL,	  'p'     },
        {      0,     0,     0,     0}
    };
    ...
    while ( ( ch = getopt_long(img_argc, img_argv, short_options, long_options, NULL  ) ) != -1 )
    {
        ...
        case 'j':	//Won't determine parity code
            ...
            break;
	 case 'p':	//special OOB size 368
            psImgInfo->m_bT24Bootstrap=1;
            break;
        ...
        case 'g':
		psImgInfo->m_bKeepYaffsTgas = atoi(optarg);
		printf("===== psImgInfo->m_bKeepYaffsTgas is %d =====\n", psImgInfo->m_bKeepYaffsTgas);
		break;
        default:
            usage();
            return FAIL;
    ...
    int bch_need_initial)
{
    ...
    for (field_index = 0; field_index < field_count; field_index++)
    {
        ...
        if (psImgInfo->m_bT24Bootstrap)
            parity_location_in_ra = ra_data + nvt_ra_size - (field_parity_size * field_count) + (field_parity_size * field_index) - 24;
        else
...
int create_image_system(
    ...
    while (!feof(fdin))
    {
        ...
        // The file size of uboot-spl MUST < ini_file.page_per_block page size
        if (page >= psgoldeninfo->m_i32PagePerBlock)
        {
            printf("ERROR: uboot-spl file size too large !\n");
            fclose(fdin);
            goto LEB_SYS_FAIL;
        }
    ...
    // pad 0xFF to unused pages till last page
    DBG_PRINTF("\tPad 0xFF for uboot-spl from block %d page %d, image file address 0x%x\n", block, page, (block * psgoldeninfo->m_i32PagePerBlock + page) * ( psgoldeninfo->m_i32PageSize + psgoldeninfo->m_i32OOBSize ));
...
int create_image_binary_file(
    ...
    //--- open output file for golder file
    sprintf(tmp, "%s/%s", psgoldeninfo->m_psOutputPath, psImgInfo->m_psImgBaseName);
    fdout = fopen(tmp, "wb");
    if (fdout == NULL)
    {
        printf("ERROR: uboot-spl file open fail: %s not found !!\n", tmp);
        return FAIL;
    }
...
if ( psImgInfo->m_i32OOBSize > 0 )
{
	...
	if (psImgInfo->m_bKeepYaffsTgas) {
		result = fread(gRa_buf, 1, psgoldeninfo->m_i32OOBSize, fdin);
	}
...
int main(int argc, char** argv)
{
    ...
    s_sGoldenInfo.m_eChip = eCHIP_NUC970;
    ...
    while ( ( ch = getopt(argc, argv, "hvO:t:c:p:o:s:b:P:" ) ) != -1 )
    {
        ...
        case 'v':	//version
            printf("nuc970ImgMaker Version: %d.%d\n" \
        ...
        case 't':	//nuc970.ini
            ...
            {
                DBG_PRINTF("nuc970.ini string size is over %d bytes", (int)sizeof(s_sGoldenInfo.m_psTurboWriterPath) );
                return FAIL;
            }
            else
    ...
    if ( s_sGoldenInfo.m_i32BlockSize < 1 )
    {
        ...
        printf("The path of nuc970.ini is invalid.\n");
        return FAIL;
